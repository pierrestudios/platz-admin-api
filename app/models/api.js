...
module.exports = Api = {
  ...
  loadDBModels: function(callback) {
    ...
    if (db) {
      ...
      NodeApp.Models.ServiceRequest.hasMany(
        NodeApp.Models.ServiceRequestProposal,
        { foreignKey: "requestId", as: "Proposals" }
      );
    ...
    this.loadDBModels(function(res) {
      if (res) {
        ...
        NodeApp.Models.User.findAll({
          ...
          if (user) {
            if (user.length) callback(user[0].dataValues);
  ...
  loadDBData: function(params, callback) {
    this.loadDBModels(function(res) {
      if (res) {
        ...
        if (params.filter)
          whereClause = Api.addWhereFilter(whereClause, params.filter);
        // console.log('includes', includes);
        if (params.key && params.key == "id" && params.value) {
          NodeApp.Models[params.model]
            .findOne({
              where: whereClause,
              include: includes
            })
